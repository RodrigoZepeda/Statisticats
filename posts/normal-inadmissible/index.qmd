...
For a gaussian distribution, the classical estimator for the mean - the sample mean $\bar{x}$ - is the "best" estimator of $\mu$ [@casella2021statistical].[^1] It is unbiased, consistent, efficient, what else can you wish for?[^2]


[^1]: It is the minimum-variance unbiased estimator (MVUE) but that's beside the point.

[^2]: Bayesian readers, please refrain from answering that retorical question 😂.
...
if (render_figure){
  ...
  ggsave("normal1.svg", width = 8, height = 6)
}
```

![Estimators of the mean of a gaussian distribution with $m = 4$ dimensions under the classical $\bar{x}$ estimator and the James-Stein estimator.](normal1.svg){fig-alt="Estimation of the mean of a gaussian distribution. Shows the realization of simulations as projection of the simulated points into each of the dimensions. The James–Stein estimators are consistently closer to the true mean." fig-align="center"}
